...
pub const OuterShell = struct {
    ...
    pub fn executeCommandParsed(self: *OuterShell, commandName: []const u8, args: []const []const u8, stdout_file: ?std.fs.File, stderr_file: ?std.fs.File) !void {
        ...
        child.stderr_behavior = if (stderr_file != null) .Pipe else .Inherit;
